...
//Multiplexer4067 mux = Multiplexer4067(s0, s1, s2, s3, x1);
AnalogMux mux(s0, s1, s2, x1);
//AnalogMux mux(4,3,2, READPIN);
...
void readMux() {
  ...
  for (int i = 0; i < N_MUX_PINS; i++) {
    Serial.print(i);
    Serial.print(": ");
    //Serial.print(map(mux.readChannel(i), 0, 1022, 0, 127));
    mux.SelectPin(muxPin[i]); // choose the pin you want to send signal to off the DeMux
    Serial.print(mux.AnalogRead());
    Serial.print("   ");
  }
